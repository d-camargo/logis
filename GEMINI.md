...
- Algoritmos de processamento: **avaliar primeiro** a solução nativa do QGIS é obrigatório, não preferência — `processing.run()` com providers `native:` e `qgis:` (ex.: `native:shortestpathpointtopoint`, `native:serviceareafrompoint`, `qgis:distancematrix`, `native:clip`, `native:joinbylocation`) ou a classe PyQGIS equivalente (como `QgsGraph` para a rede, acima); código próprio só entra quando a nativa não serve:
  ...
  - verificado estaticamente por `test_native_rules.py`; a lista `LEGACY_ALLOWED` está zerada (nenhum algoritmo usa `QgsCoordinateTransform` cru) e nenhum arquivo novo pode entrar nela.
...
- Comprimento e área sempre pelo `QgsDistanceArea` elipsoidal (`setSourceCrs` + `setEllipsoid(crs.ellipsoidAcronym())`, padrão de `urban_network_density.py`; helper pronto `logis/core/crs_transform.py:length_meter`), medido na geometria original — nunca `geometry.length()`/`.area()` em `logis/algorithms/` (planar, na unidade de mapa do SRC da camada; verificado estaticamente por `test_native_rules.py`, sem lista de exceções, 0.6.5). Devolver saídas em EPSG:4674.
