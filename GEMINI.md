...
- Grafos e caminhos mínimos: usar `QgsGraph`, `QgsGraphBuilder`, `QgsGraphAnalyzer` (Dijkstra) do módulo `qgis.analysis`. **Não usar** networkx, igraph, OSMnx. Exceção registrada: `logis/core/routing/arc_routing.py` (CPP/RPP/CARP) usa grafo de arestas em dicionário + Dijkstra próprio (`heapq`), porque opera sobre um **multigrafo de arestas** (duplicação para eulerianizar, Hierholzer, path-scanning) que o `QgsGraph` não modela (0.6.5). Continua valendo que nenhum outro módulo pode ter Dijkstra próprio.
- Algoritmos de processamento: **avaliar primeiro** a solução nativa do QGIS é obrigatório, não preferência — `processing.run()` com providers `native:` e `qgis:` (ex.: `native:shortestpathpointtopoint`, `native:serviceareafrompoint`, `qgis:distancematrix`, `native:clip`, `native:joinbylocation`) ou a classe PyQGIS equivalente (como `QgsGraph` para a rede, acima); código próprio só entra quando a nativa não serve:
  ...
  - camada de entrada em outro SRC, dentro de algoritmo de Processing, é lida já reprojetada: `source.getFeatures(QgsFeatureRequest().setDestinationCrs(crs, context.transformContext()))` ou `native:reprojectlayer`; `QgsCoordinateTransform` cru em `logis/algorithms/` é proibido — a exceção é o helper conferido `logis/core/crs_transform.py` (`checked_transform`/`transform_points`/`transform_bbox`/`read_points_in_crs`/`read_lines_in_crs`/`length_meter`) quando for preciso transformar pontos soltos; `read_points_in_crs` é a forma recomendada de ler uma camada de pontos/polígonos já reprojetada e conferida (detecta a feição que não transforma e volta com geometria vazia em silêncio); `read_lines_in_crs` é o mesmo padrão para linhas (nós de grafo em coordenadas métricas); `length_meter` mede comprimento elipsoidal em metros na geometria original, sem reprojetar (0.6.5);
  - verificado estaticamente por `tests/test_native_rules.py`; a lista `LEGACY_ALLOWED` está zerada (nenhum algoritmo usa `QgsCoordinateTransform` cru) e nenhum arquivo novo pode entrar nela.
...
- Testes vivem em `tests/` (é assim que o gate do planexec os acha; na raiz, o gate cai no `make test`, que só checa sintaxe).
...
- Comprimento e área sempre pelo `QgsDistanceArea` elipsoidal (`setSourceCrs` + `setEllipsoid(crs.ellipsoidAcronym())`, padrão de `urban_network_density.py`; helper pronto `logis/core/crs_transform.py:length_meter`), medido na geometria original — nunca `geometry.length()`/`.area()` em `logis/algorithms/` (planar, na unidade de mapa do SRC da camada; verificado estaticamente por `tests/test_native_rules.py`, sem lista de exceções, 0.6.5). Devolver saídas em EPSG:4674.
...
- Padrões de empacotamento e documentação (validados estaticamente por `tests/test_packaging.py`):
