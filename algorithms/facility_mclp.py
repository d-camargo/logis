...
"""
"""
Algoritmo de processamento para localização de instalações via Cobertura Máxima (MCLP).
"""
...
try:
    from ..core.location.facility import solve_mclp
...
class FacilityMCLP(QgsProcessingAlgorithm):
    """
    Algoritmo QGIS Processing para resolver o problema de localização de cobertura máxima (MCLP)
    utilizando a heurística gulosa de Church & ReVelle (1974).

    Maximiza a demanda total coberta dentro de um raio/tempo máximo de atendimento (max_distance),
    selecionando até p instalações candidatas.
    ...
    Limite de Complexidade:
        Complexidade de Tempo: O(p * M * N) onde N é demanda e M candidatos.
    ...
    def tr(self, string):
        ...
        return QCoreApplication.translate("FacilityMCLP", string)
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                self.P_FACILITIES,
                self.tr("Número máximo de instalações (p)"),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # 5) Resolução da Cobertura Máxima (MCLP)
        feedback.pushInfo(self.tr("Executando a otimização MCLP (Church & ReVelle)..."))
        try:
            selected_indices, covered_demand, total_demand, coverage_ratio, covered_mask = solve_mclp(
        ...
        feedback.pushInfo(
            self.tr("MCLP concluído. Instalações: {sel} | Demanda Coberta: {cov:.2f}/{tot:.2f} ({ratio:.1%})").format(
                sel=selected_indices, cov=covered_demand, tot=total_demand, ratio=coverage_ratio
            )
        )
        ...
        for i in range(num_demands):
            if covered_mask[i]:
                # Find closest selected facility that covers demand i
        ...
        if sink_ass is not None:
            for i, feat in enumerate(demand_features):
                is_cov = 1 if covered_mask[i] else 0
                cand_idx = assigned_facility[i]
                cost = cost_to_facility[i]
                w_cost = demand_weights[i] * cost if is_cov else 0.0
                new_feat = QgsFeature(feat)
                attrs = feat.attributes()
    ...
    def name(self):
        return "facility_mclp"

    def displayName(self):
        return self.tr("Localização de Cobertura Máxima (MCLP)")
    ...
    def shortHelpString(self):
        return self.tr(
            "Resolve o problema de localização de cobertura máxima (MCLP) utilizando a heurística "
            "gulosa de Church & ReVelle (1974).\n\n"
            "Seleciona até p instalações candidatas que maximizem o peso da demanda atendida "
