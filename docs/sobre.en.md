...
logis is at version **0.1.10**, marked as `experimental` in the official QGIS plugin repository. In this phase, internal APIs, algorithm identifiers, and output formats may change between releases without backward compatibility guarantees — experimental plugins must be enabled in the Plugin Manager to install it (see [Installation Guide](guias/instalacao.md)). The license is **GPL-3.0**, inherited from GisBR logic.
